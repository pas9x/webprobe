...
class PingCheckerWindows implements Checker {
    public final String host;
    public final int count;
    protected Process proc;
    protected String stopReason;

    public PingCheckerWindows(String host, int count) {
        this.host = host;
        this.count = count;
    }
    ...
    public CheckResult check() {
        ...
        cmd.add("cmd.exe");
        cmd.add("/c");
        cmd.add("chcp 437 && ping -n "+ this.count + " " + this.host);
    ...
    protected PingResult formatCheckResult(int pingExitCode, String pingOutput) {
        ...
        if (this.stopReason == null) {
            LinkedList<String> matches = RegexpHelper.match("\\((\\d{1,3}(\\.\\d{1,8})?)\\% loss\\)", pingOutput);
